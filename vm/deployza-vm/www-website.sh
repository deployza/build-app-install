...
#
# THIS IS A NEAR-COPY OF ziniapps-www.sh, WITH ONE DELIBERATE DIFFERENCE
#
...
# /etc/nginx/site.d/. Unlike ziniapps-www.sh it DOES include
# /etc/nginx/app.d/*.conf, because this host carries a path-prefixed app
# alongside the marketing site — see below.
#
# THE API DOCS ARE NOT THIS SCRIPT'S JOB — SEE www-apidocs.sh
#
# This script used to do a second, unrelated job: build and serve the MkDocs API
# documentation at a path under this same hostname. That is now www-apidocs.sh,
# a per-PATH app that owns the docs root, the git checkout, the docs-refresh
...
# hostname has its own server_name block it no longer falls through to the
# image's `_` default server, so without that include the app.d drop-ins simply
# do not resolve on www.deployza.com — exactly the consequence CLAUDE.md calls
# out, and the same reason ziniapps-go.sh carries the line. REMOVING IT SILENTLY
...
# response repeats the three headers rather than inheriting them: add_header in
# a nested location REPLACES any inherited set rather than adding to it — which
# is also why the included app.d drop-ins carry their own copies rather than
# relying on anything set here.
#
...
#   4. ensures /etc/nginx/app.d/ exists             (its server block includes it)
...
# <clone>/vm/deployza-vm/www-website.sh — so the name is already implied.) Only APP_ENV
...
# are this app's own.
#
# NOTE: unlike ziniapps-www.sh, this script DOES use NGINX_APP_D — its server
# block includes the per-path drop-ins so that www-apidocs.sh's /api-docs/
# resolves on this hostname (see write_nginx_conf). INCLUDE_MARKER is shared BY
# VALUE with ziniapps-go.sh/ziniapps-www.sh: whichever per-HOST script runs
...
require_tools() {
  ...
  # No git/gcloud/mkdocs check here any more: those belong to the API docs,
  # which are a separate app (www-apidocs.sh) with its own require_tools.
...
This dir is created by the per-HOST deploy scripts in build-ops/vm/
...
# config error (the *.conf wildcard may legally match zero files; the directory
# still has to be there). Creating it is idempotent, and www-apidocs.sh does the
# same, so whichever of the two runs first wins and the other finds it already
# present.
ensure_app_d() {
...
# fully determined by values we already have (SERVER_NAME and DOC_ROOT).
#
# `root` rather than `alias`: this block serves the whole host from one tree, so
# the URI appends to the root directly and `root` states that plainly. (An app.d
# drop-in mapping a URL prefix to a differently-named dir uses `alias` instead —
# see www-apidocs.sh.)
#
...
#
# include app.d: LOAD-BEARING, not defensive. Once this hostname has its own
# server_name block it stops falling through to the image's `_` default server,
# so without this line every app.d drop-in — today, www-apidocs.sh's /api-docs/
# — 404s on www.deployza.com while `nginx -t` still passes. Longest-prefix
# matching means those locations win over `location /` below, so the WAR's root
# handler never sees their requests. Same reasoning as ziniapps-go.sh's copy of
# the line; ziniapps-www.sh deliberately omits it because that host really does
# carry exactly one product.
#
# Caching: the three no-store-ish headers go on EVERY response this block itself
# serves — the WAR's files and the 404 page alike — so nothing is reused from
# cache without a revalidation round-trip and a redeploy takes effect
# immediately. `always` makes them apply to error responses too (add_header
# otherwise covers only 2xx/3xx). Note that add_header in a nested location
# REPLACES any inherited set rather than adding to it, so EACH location that can
# produce a response repeats them — which is also why the included app.d
# drop-ins carry their own copies rather than relying on anything set here.
write_nginx_conf() {
  echo "Writing nginx server block ${NGINX_CONF}..."

  cat >"$NGINX_CONF" <<EOF
# ${APP_NAME} — generated by build-ops/vm/deployza-vm/${APP_NAME}.sh. Do not edit by
...
server {
    ...
    # Path-prefixed apps on this host (www-apidocs.sh's /api-docs/ today).
    # Load-bearing — see this function's header for what silently breaks
    # without it.
...
main() {
  ...
  ensure_app_d            # one-time: create app.d, which the server block includes
  ...
  echo "Path-prefixed apps on this host (app.d drop-ins, e.g. www-apidocs.sh's"
  echo "  /api-docs/) are served by this block too — see write_nginx_conf."
}
