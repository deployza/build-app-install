...
# -----------------------------------------------------------------------------
# assess-exam.sh — app deploy script (the <APP_NAME>.sh that is PUSHED to
# the VM and run there). assess-exam is a STATIC UI bundle: no database, no
...
#       sudo bash apps/assess-exam/assess-exam.sh <APP_ENV> 2>&1 | tee /tmp/assess-exam.log
#
...
# <clone>/vm/ziniapps-vm/assess-exam.sh — so the name is already implied.) Only APP_ENV varies
...
write_nginx_conf() {
  ...
  cat >"$NGINX_CONF" <<EOF
# ${APP_NAME} — generated by build-ops/vm/ziniapps-vm/${APP_NAME}.sh. Do not edit by
