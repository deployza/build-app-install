...
# -----------------------------------------------------------------------------
# assess-ui.sh — app deploy script (the <APP_NAME>.sh that is PUSHED to
# the VM and run there). assess-ui is a STATIC UI bundle: no database, no
...
# <clone>/vm/ziniapps-vm/assess-ui.sh — so the name is already implied.) Only APP_ENV varies
