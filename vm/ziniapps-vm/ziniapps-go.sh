...
#       sudo bash vm/ziniapps-vm/ziniapps-go.sh <APP_ENV> 2>&1 | tee /tmp/ziniapps-go.log
#
...
# <clone>/vm/ziniapps-vm/ziniapps-go.sh — so the name is already implied.) Only APP_ENV
...
This dir is created by the ziniapps-* deploy scripts in build-ops/vm/,
